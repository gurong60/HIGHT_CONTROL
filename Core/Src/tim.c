...
{
    if (ch > 3 || brightness > PWM_ARR)
        return;

    /* Linear mapping: brightness 0-999 �� CCR 0-PWM_ARR */
    uint32_t ccr = (uint32_t)brightness ;
...
void Set_PWM_En(uint32_t Channel, uint8_t start)
{
    // 限制通道 0~3（CH1~CH4）
    if (Channel > 3) return;

    // 计算 CCER 偏移位：0/4/8/12（和你原代码 Channel*4 完全一致）
    uint32_t shift = Channel << 2; 
    ...
    if (start)
    {
        // 1. 开启通道输出
        TIM1->CCER |= mask;
        // 2. 高级定时器必须开启：主输出使能 MOE(bit15) → 致命关键！
        TIM1->BDTR |= TIM_BDTR_MOE;
        // 3. 确保定时器计数器运行（可选，初始化已开就不用）
...
// 启用 DWT 周期计数器（用于微秒级精准计时）
uint32_t micros(void)
{
    return (uint32_t)TIM17->CNT;
}
/* USER CODE END 1 */
